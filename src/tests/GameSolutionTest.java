...
public class GameSolutionTest {
	...
	void solutionChecker() {
		...
		assertTrue(PlayerActions.accusation(new 
				Guess(testCard1, testCard2, testCard3)));
		...
		assertFalse(PlayerActions.accusation(new 
				Guess(incorrectPerson, testCard2, testCard3)));
		assertFalse(PlayerActions.accusation(new 
				Guess(testCard1, incorrectRoom, testCard3)));
		assertFalse(PlayerActions.accusation(new 
				Guess(testCard1, testCard2, incorrectWeapon)));
...
//	@Test
	void insureDealingIsUpdatingSeenList() {
		...
		assertTrue(testPlayer1.guessLogic
				.checkIfSeen(testPlayer1.getHand().get(1)));
		assertFalse(testPlayer1.guessLogic
				.checkIfSeen(testPlayer2.getHand().get(1)));
		assertFalse(testPlayer1.guessLogic
				.checkIfSeen(testPlayer2.getHand().get(2)));
		testPlayer1.updateHand(testPlayer2.getHand().get(1));
		assertTrue(testPlayer1.guessLogic
				.checkIfSeen(testPlayer2.getHand().get(1)));
		testPlayer1.guessLogic.addListToSeen(testPlayer3.getHand());
		assertTrue(testPlayer1.guessLogic
				.checkIfSeen(testPlayer3.getHand().get(1)));
		assertTrue(testPlayer1.guessLogic
				.checkIfSeen(testPlayer3.getHand().get(2)));
		
		}
			
	//Test a computer making a suggestion, 
	//and receiving a reply form another computer player
	...
	void testingReplyBoolean() {
		...
		testingPlayerList.add(new 
				ComputerPlayer(testDeck1.getDeck().get(0)));
		testingPlayerList.add(new 
				ComputerPlayer(testDeck1.getDeck().get(2)));
		
		
		DeckActions.dealDeck(testDeck2, testingPlayerList
	...
	void disproveSuggestionTests() {
		...
		ComputerPlayer testPlayer1 = new 
				ComputerPlayer(testDeck1.getDeck().get(0));
		ComputerPlayer testPlayer2 = new 
				ComputerPlayer(testDeck1.getDeck().get(1));
		ComputerPlayer testPlayer3 = new 
				ComputerPlayer(testDeck1.getDeck().get(2));
		...
		for(ComputerPlayer checkReply: testingPlayerList) {
			if(checkReply.checkForReply(p1Guess)) {
				testReplyList.add(new Card(checkReply
						.generateReply(p1Guess)));
		}}
