...
public class SeenPanel {
	...
	public static JPanel populateSeenList() {
		...
		UIPlayerControl.playerList.forEach(player->{
			player.getHand().stream().forEach(card->{
				...
				jTextMap.get(card.getCardName())
							.setName(card.getCardName());
				jTextMap.get(card.getCardName())
							.insert("  " + card.getCardName(), 0);
				jTextMap.get(card.getCardName())
							.setBackground(player.getColor());
				jTextMap.get(card.getCardName())
							.setPreferredSize(new Dimension(SEEN_PANEL_WIDTH, 40));
				jTextMap.get(card.getCardName())
							.setVisible(isVISABLE);
				jTextMap.get(card.getCardName())
							.setFont(new Font("Helvetica Neue", Font.PLAIN, 10));
			});
		});
		...
		panelMap.get(CardType.NONE)
					.setPreferredSize(new Dimension(SEEN_PANEL_WIDTH,125));
		...
		for(Card card: UIPlayerControl.getHumPlayer().getHand()){
			...
			GBC.gridy = indexer++;
			panelMap.get(CardType.NONE).add(jTextMap
					.get(card.getCardName()),GBC);
			indexer++;
		}
		...
		for (CardType type: typeList) {
			...
			tempP.add(new JLabel("    Seen " + type.toString()
								.toLowerCase() + "s:"));
		...
		panelMap.get(CardType.NONE).setBorder(BorderFactory
		...
		panelMap.get(CardType.PERSON).setBorder(BorderFactory
									.createLineBorder(Color.BLACK));
		...
		panelMap.get(CardType.WEAPON).setBorder(BorderFactory
									.createLineBorder(Color.BLACK));
	...
	public static void addCard(Card newCard) {
		SeenPanel.UIWatchList.addElement(newCard);
	}
